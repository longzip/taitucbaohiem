<template>
  <div class="q-pa-md">
        <ListHeader  bgcolor="bg-orange-4">
            Danh sách khách hàng tái tục BHXH {{khachHangTaiTucs.length}}/Tổng: {{tongCong.toLocaleString()}}
        </ListHeader>
        <q-list v-for="khachHang in khachHangTaiTucs" :key="khachHang.maSoBHXH">
            <q-item>
                ...
                <q-item-label caption>Số tiền: {{ parseInt(khachHang.soPhaiDong).toLocaleString() }} / {{ khachHang.phuongThuc}} tháng</q-item-label>
                </q-item-section
            ></q-item>
    ...
    methods: {
        async loadData(){
            ...
            if(!error){
                ...
                this.tongCong = await result.items.map(t=>t.soPhaiDong).reduce(
