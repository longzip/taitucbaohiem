...
const GRAPHQL_API_URL =
  "https://hotham.vn/wordpress/rYkOy1HCCRD0JZZcrshVYaUR39QfcG15QWUC437BMM5Pk3gNLu";
...
(async function () {
  try {
    if (typeof pdfjsLib === "undefined") {
      throw new Error(
        "Thư viện pdf.js chưa được tải. Vui lòng tải lại trang và thử lại."
      );
    }
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      const hoTenMatch = pageText.match(
        /Họ và tên người nộp\s*:\s*(.*?)\s*Địa chỉ/
      );
      ...
      const noiDungThu =
        (pageText.match(
          /Nội dung thu\s*:\s*(.*?)\s*(?:Số tiền thu)/s
        ) || [])[1]
          ?.trim()
          .replace(/\s+/g, " ") || "";
      ...
      const chuoiThoiGianMatch = pageText.match(
        /từ (?:ngày|tháng)\s*([\d\/]+)\s*đến (?:ngày|tháng)\s*([\d\/]+)/i
      );
      let tuThoiGian = null;
      ...
      if (chuoiThoiGianMatch) {
        ...
        if (denParts.length === 2) {
          ...
          denThoiGian = `${endDate.getFullYear()}-${(endDate.getMonth() + 1)
            .toString()
            .padStart(2, "0")}-01`;
        } else {
      ...
      const payload = {
        ...
        maXacNhan,
        maSoBhxh: maSoBHXH,
        noiDungThuDayDu: noiDungThu,
        loaiBienLai,
        ngayGiaoDich: ngayGiaoDichApi,
        nhanVienThu: nhanVien,
        soThangDong,
        tuThoiGian,
        denThoiGian,
        soTienThu,
        phanTramHoaHong: loaiBienLai === "BHXH" ? 4.9 : 2.64,
        userId: nhanVien === "142010_giaodichvien" ? 3152 : 1,
      };
...
✅ Lưu lại mã và biên lai để đảm bảo quyền lợi BHXH, BHYT trọn đời\n📞 Mọi thắc mắc, liên hệ: 0978.333.963 (Zalo – chị Hồ Thị Thắm)\n🌐 Website: www.hotham.vn`;
    ...
    const graphqlQuery = {
      query: `
        ...
        `,
      variables: {
        input: { danhSachBienLai: allPayloads },
      },
    };

    const capNhatBienLaiBH = await fetch(GRAPHQL_API_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(graphqlQuery),
    });
    const result = await capNhatBienLaiBH.json();
    if (result.data && result.data.capNhatDanhSachBienLaiBH.success) {
      notify(
        `Hoàn tất! Đã gửi thành công ${allPayloads.length} biên lai về máy chủ.`
      );
    } else {
      console.error(
        "Lỗi GraphQL:",
        result.errors || result.data.capNhatDanhSachBienLaiBH.message
      );
    }
